...
class TurnEngine:
    ...
    async def _authorize(self, tool_call: ToolCall) -> "AsyncIterator[Event | bool]":
        ...
        if self._downloaded_target(tool_call) is not None and (
            ...
            allowed = False
            reason = f"此文件是 agent 本会话中下载的 —— {provenance_note}"
            decision = replace(
                decision,
                allowed=False,
                reason=reason,
        ...
        if allowed and decision.reason == REASON_FULL_ACCESS:
        ...
        if allowed and decision.reason in (REASON_TRUSTED_RULE, REASON_TRUSTED_SERVER):
            origin = "trusted_rule" if decision.reason == REASON_TRUSTED_RULE else "trusted_server"
        ...
        if allowed and decision.reason == REASON_RUN_GRANT:
