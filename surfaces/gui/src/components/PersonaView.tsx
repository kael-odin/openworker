...
export function PersonaView({
  ...
  useEffect(() => {
    ...
    getPersonaDetail(personaId)
      ...
      .catch(() => live && setError(t("personaview.err_load")));
    getConnectors()
  ...
  const exportBundle = async () => {
    ...
    setMsg(r.ok ? t("personaview.exported_to", { path: r.path || "" }) : r.error || t("personaview.export_failed"));
  };
  ...
  const header = (
    <div className="h-12 shrink-0 px-5 flex items-center gap-3 border-b border-line bg-paper">
      ...
      <span className="text-[13px] font-semibold">{t("personaview.header")}</span>
    </div>
  );
  ...
  if (error || !detail) {
    return (
      <main className="flex-1 min-w-0 flex flex-col bg-paper">
        {header}
        <div className="p-12 text-center text-faint text-[13px]">{error || t("personaview.loading")}</div>
      </main>
    );
  }
        ...
        <div className="max-w-3xl mx-auto px-7 py-6 space-y-6">
          ...
          <header className="flex items-start gap-3.5">
            ...
            <div className="ml-auto flex items-center gap-2">
              <span className="text-[12px] text-muted">{detail.enabled ? t("personaview.enabled") : t("personaview.disabled")}</span>
              <Toggle checked={detail.enabled} onChange={toggleEnabled} title={t("personaview.enable_title")} />
            </div>
          </header>
                  ...
                  <div className="flex items-center gap-2">
                    {mediaUrls.length > 1 && (
                      <button
                        ...
                        aria-label={t("personaview.prev_screenshot")}
                    ...
                    <img
                      src={mediaUrls[shot]}
                      alt={`${detail.name} ${t("personaview.screenshot_n", { n: shot + 1 })}`}
                    ...
                    {mediaUrls.length > 1 && (
                      <button
                        className="w-7 h-7 rounded-full border border-line bg-panel text-muted hover:text-ink hover:border-lineStrong shrink-0"
                        aria-label={t("personaview.next_screenshot")}
                  ...
                  {mediaUrls.length > 1 && (
                    <div className="flex justify-center gap-1.5 mt-2">
                      {mediaUrls.map((_, i) => (
                        <button
                          key={i}
                          aria-label={t("personaview.screenshot_n", { n: i + 1 })}
                          className={
                  ...
                  <div className="flex items-center gap-3 px-4 py-3" key={r.key}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {r.kind === "mcp" ? (
                          <span className={TAG_MCP}>{t("personaview.tag_mcp")}</span>
                        ) : r.tier === "core" ? (
                          <span className={TAG_CORE}>{t("personaview.tag_core")}</span>
                        ) : null}
                      </div>
                    ...
                    <span className={COL_STATUS}>
                      {r.connected ? (
                        ...
                        >
                          {r.kind === "mcp" ? t("personaview.add") : t("personaview.connect")}
                        </button>
                      )}
                    </span>
                    <span className={COL_ENABLE}>
                      {r.dflt ? (
                        <Toggle
                          ...
                          title={
                            r.connected
                              ? t("personaview.on_by_default")
                              : t("personaview.connect_first")
                          }
                        />
                      ) : (
          ...
          {detail.tools.length > 0 && (
            <section>
              <div className={`${SEC_H} mb-1.5`}>{t("personaview.advanced")}</div>
              <div className="rounded-xl2 border border-line bg-panel">
                ...
                >
                  ...
                  />
                  <span className="text-[13px]">{t("personaview.tool_calls")}</span>
          ...
          <section className="flex flex-wrap gap-x-8 gap-y-2 text-[12.5px]">
            ...
            {detail.default_permission_mode && (
              <div>
                <span className="text-faint">{t("personaview.default_mode")}</span> · {detail.default_permission_mode}
              </div>
            )}
            <div>
              <span className="text-faint">{t("personaview.workspace")}</span> ·{" "}
              {detail.requires_folder ? t("personaview.picked_folder") : t("personaview.scratch")}
            </div>
          </section>
          ...
          <section className="border-t border-line pt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-[12.5px]">
            ...
            >
              {detail.default ? t("personaview.default_for_new") : t("personaview.make_default")}
            </button>
            {!detail.builtin && (
              <button className={BTN_BORDERED} data-testid="persona-export" onClick={exportBundle}>
                {t("personaview.export")}
              </button>
            )}
            {!detail.builtin &&
              (confirmDel ? (
                <span className="flex items-center gap-1.5">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (r.ok) onBack?.();
                      else setMsg(r.error || t("personaview.delete_failed"));
                    }}
                  >
                    {t("personaview.delete")}
                  </button>
